...
你可以在 https://nuxt-view-transitions.surge.sh 上查看演示，以及在[StackBlitz 上的源码](https://stackblitz.com/edit/nuxt-view-transitions)。
...
1. 以前可以传递 `x-nuxt-no-ssr` 请求头（未记录）来强制进行 SPA 渲染。我们现在默认禁用了这一行为，但你可以通过将 `experimental.respectNoSSRHeader` 设置为 true 将其恢复。或者，你也可以在服务端设置 `event.context.nuxt.noSSR` 来强制进行 SPA 渲染。
2. 我们已[移除（已弃用的）`#head` 别名](https://github.com/nuxt/nuxt/pull/20111)，并且默认禁用了 [@vueuse/head 行为的 polyfill](https://github.com/nuxt/nuxt/pull/20131)。它仍然可以通过设置 `experimental.polyfillVueUseHead` 来启用。
3. 我们已[移除（已弃用的）`experimental.viteNode` 选项](https://github.com/nuxt/nuxt/pull/20112)。现在可以改为通过 `vite.devBundler` 进行配置。
4. 我们已[弃用在没有 `public` 键的情况下访问公共运行时配置](https://github.com/nuxt/nuxt/pull/20082)。这是一个与 Nuxt 2 的未记录兼容性措施，我们计划在 v3.5 中将其完全移除。
5. 为修复 vue-router 集成中的一个 bug，我们现在生成了略有不同的路径匹配语法。如果你依赖的是精确生成的路径，请查看 https://github.com/nuxt/nuxt/pull/19902 了解更多信息。
