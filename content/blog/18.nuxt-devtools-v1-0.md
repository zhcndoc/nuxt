...
- **可在所有浏览器中使用**，甚至在移动设备上也可以！- 浏览器扩展 DevTools 的能力受各浏览器提供的 API 限制，而且维护多个扩展也需要大量精力。这种方式可以让我们更专注于功能和特性，同时让任何浏览器和设备上的用户都能使用。
- **构建工具集成** - 传统的浏览器扩展 DevTools 只能访问应用的运行时上下文，无法访问构建工具。随 Nuxt 一起提供的 DevTools 使我们能够与构建工具通信，并提供更多洞察和功能。
- **避免布局偏移** - 将 DevTools 作为浮动面板可以避免切换 DevTools 时产生的布局偏移。
...
时间线是一种用于检查每个可组合项在何时以及如何被调用的工具。不同于浏览器 DevTools 的性能工具，此标签页只检查高层级的可组合项，并结合路由导航等其他事件，这更贴近日常使用。它还会记录每次调用的参数和返回值，因此您可以更好地了解底层发生了什么。
...
我们对 Nuxt DevTools 如何帮助您构建更好的应用并改善开发者体验感到兴奋。展望未来，我们正在设想比 Nuxt DevTools 本身更宏大的东西。我们相信，拥有这样与框架相关的 DevTools 是提升开发者体验的前进方向。我们也看到，这类工具的许多部分实际上可以在不同工具之间共享和复用。于是我们提出了 DevTools Kit 的想法。

### DevTools Kit

DevTools Kit 是一个仍处于早期头脑风暴阶段的通用协议构想。我们设想，在最理想的情况下，DevTools 的每个特性都应该是 **可组合的、可扩展的、协作的**。元框架可以为其特定需求构建自己的特性，而与 Web 相关的通用工具可以在不同框架之间共享并协作。

![DevTools Kit](/assets/blog/devtools/slide-devtools-kit.png){.border.border-gray-200.dark:border-gray-700.rounded-lg}

想象一下，我们可以拥有这一切特性，每个都是一个独立的包。我们可以拥有与 Web 相关的通用工具，如 SEO、可访问性、PWA、静态资源等。然后是与底层构建工具相关的特性，如 Vite 构建分析器、Vite 检查器或 Webpack 可视化工具等。最后，我们还可以拥有框架和元框架特定的工具，如 Vue 组件视图，或 Nuxt 服务器 API 游乐场等。
...
那时，Vue DevTools 将由通用 Web 特性和 Vue 特定特性组成。而 Nuxt DevTools 本质上将由上述特性组成，继承 Vue DevTools 的所有特性，并在其之上添加 Nuxt 特定特性。
