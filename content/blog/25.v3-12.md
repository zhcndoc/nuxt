---
title: Nuxt 3.12
description: Nuxt 3.12 已发布 - 充满改进，并为 Nuxt 4 铺平道路！
...
我们正在迈向 Nuxt 4 的发布之路，但在 Nuxt v3.12 之前我们没有停下脚步。
...
我们一直在逐步发布[Nuxt Scripts](https://scripts.nuxt.com/)。它目前处于公共预览阶段，但我们将很快进行公开发布，因此我们已经为使用时会提示安装 `@nuxt/scripts` 模块的 composables 添加了一些存根。

👉 请密切关注正式发布以及解释更多内容的文章！

## 🌈 自动注册层和 bug 修复

就像 `~/modules` 一样，现在 `~/layers` 目录中的任何层都将自动注册为项目的层（[#27221](https://github.com/nuxt/nuxt/pull/27221)）。
